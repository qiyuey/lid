...
/// `SleepDisabled` flag without an admin prompt. The helper owns and persists
/// the user-visible sleep-prevention state; the app only reads and writes that
/// state over XPC.
@objc public protocol LidHelperProtocol {
    ...
    /// Read the helper-owned sleep prevention state. reply: (enabled).
